...
ES.MapView.UserLayer = L.MapLib.MapMaster.MapOpr.extend({
    ...
    oOption: {
        ...
        cHtml:
        '<div class="ex-monitor-mapicon-user  {Icon} ">' +
        '   <i></i>' +
        '   <div class="user-body">' +
        '       <div class="pin-dome"></div>' +
        '       <div class="pin-number">{StaffName}</div>' +
        '   </div>' +
        '</div>'
    },
    ...
    _loadOn: function () {
        ...
        //给界面赋值，并画工地
        this._oParent.on('MapView:ShowUserLayer.DrawLayers', this.drawSites, this);

        // 清除工地
        this._oParent.on('MapView:ShowUserLayer.removeLayers', this.removeLayers, this);
    ...
    drawSites: function (oData) {
        ...
        for (var i = 0; i < oData.aoData.length; i++) {
            ...
            this._drawMarker(oData.aoData[i]);
        }
    },
    ...
    _drawMarker: function (oPosInfo) {
        ...
        if (!this._oUserGroup || !oPosInfo) return;

        var oLayer = this.findLayer(this._oUserGroup, oPosInfo.StaffId);
        if (oLayer) {
            return oLayer;
        }

        var oIcon = this._getIcon( ES.Util.template(this.oOption.cHtml, {Icon:'user-body',StaffName:oPosInfo.StaffName}));

        var oMarker = L.marker([oPosInfo.Lat,oPosInfo.Lon], { icon: oIcon });

        oMarker.cId = oPosInfo.StaffId;
