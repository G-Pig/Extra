...
ES.Muck.Grid = ES.Common.BaseJqGrid.extend({
    setColumns:function() {
        var list = [
            ...
            {
                label: '关联项目', name: 'RfNames', align: 'center', width: 100,
                formatter: function formatHtml(cellValue, options, rowObject) {

                    var content = '';

                    content += '<button class="ec-btn ec-btn-xs ec-btn-default ec-radius view"><i class="ec-icon-eye view"></i> 详情 </button>';
    ...
    initClick: function (e, oModel) {
        ...
        if ($(e.target).hasClass('view')) {
            ...
            this._oParent.oEditD.hideFooter();
            this._oParent.oEditD.bFootHide = true;
            this._oParent.oEditD.editShow({Id: oModel.Id, TypeId:oModel.Type});
        }
    }
});
