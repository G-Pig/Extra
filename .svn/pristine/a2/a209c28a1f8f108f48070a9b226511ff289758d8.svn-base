...
ES.MapControl.ESMapFull = ES.Evented.extend({
    ...
    initialize: function (oMapBase, options) {
        ...
        //记录最近一次的div Z-index
        this._lastZIndex = 0;
        this.$_oPContainer = $('.' + this.oOption.acParentDivClass.join('.')).eq(0);
    ...
    //加载工具事件，初始化工具栏
    initUI: function () {
        ES.initTag(this.$_oPContainer, this.oUIConfig);
    ...
    initCityHandler: function (oData) {
        ...
        this.$_oContainer.find('ul>li>a').bind('click', this, function (e) {
            ...
            if(oItem.Lat &&  oItem.Lng){
                e.data._oMap.flyTo([oItem.Lat, oItem.Lng],  this.oOption.nZoom);
            }
...
ES.MapControl.ESMapSearch = ES.Evented.extend({
    ...
    searchPoiHandler: function (oData) {
        ...
        if (!woData || oData.status === 0 || oData.count <= 0) {
            return;
        }
        // 加载数据
...
ES.Common.BaseDialog = ES.Evented.extend({
    ...
    initialize: function (oParent, oOption,oDOption) {
        ...
        ES.extend(this.oDOption,oOption);
    ...
    //this.oDialog.afterOpen=function(){}
    // 注册窗体事件
    initEvent: function () {
        if (!this.oDialog) {
            return;
    ...
    saveHandler: function (oTemp) {
        ...
        if (oData && oData.IsSuccess) {
            ES.aSucess(bAdd ? ES.Lang.BaseDialog[10] : ES.Lang.BaseDialog[20]);
            ...
            ES.aErr(ES.template(bAdd ? ES.Lang.BaseDialog[21] : ES.Lang.BaseDialog[21], oData));
...
ES.MenuFrame.NavBottom = ES.Evented.extend({
    ...
    showMenu: function ( oData) {
        ...
        //// 判断是否点击 了菜单，如果点击
        //if(!this.$_oContainer.find('ul[data-parent="'+oData.cId+'"]').find('li').hasClass('ec-active'))
        //{
...
ES.TabPanel = ES.Evented.extend({
    ...
    cHtml : '<iframe src="{cUrl}" class="ex-layout-mainframe" name="MainFrame" id="{cId}" allowfullscreen mozallowfullscreen webkitallowfullscreen frameborder="0"></iframe>'
});
