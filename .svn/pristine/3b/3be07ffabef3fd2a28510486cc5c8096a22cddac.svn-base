...
ES.MapView.MapUserLive = ES.MapView.MapLive.extend({
    ...
    _drawLiveHis: function (oPosInfo) {
        ...
        if (!oLineLayer) {
            //创建线图层
            var oPloyLine = L.polyline([oLatLng], ES.MapView.oConfig.oLiveLineConfig);
        ...
        //创建轨迹图层
        if (oPrePosInfo) {
            var oTrackLayer = L.circleMarker([oPrePosInfo.Lat, oPrePosInfo.Lng], ES.MapView.oConfig.oLiveCircleMarkerConfig);
    ...
    _createLive: function (oPosInfo) {
        ...
        var oOption = L.extend({ radius:100 },  ES.MapView.oConfig.oLiveCircleConfig)
