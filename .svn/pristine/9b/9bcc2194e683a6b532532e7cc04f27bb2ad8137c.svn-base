...
ES.MapView.AssetMonitor = ES.MapView.BaseMonitor.extend({
    ...
    initMenuBoxHandler: function (oData) {
        ...
        for(var i = 0;i< oData.rtnData.Tech.length;i++) {
            oData.rtnData.Tech[i].FeatureId = oData.rtnData.Tech[i].TypeId;
            oData.rtnData.Tech[i].FeatureName = oData.rtnData.Tech[i].TypeName;
            // 初始化图表和菜单
    ...
    initEvent: function () {
        ...
        this.$_oRoadBox.find('.ex-monitor-wbox').bind('click', function () {
            ...
            for (var i = 0; i < self.oBusData.TechGroup.length; i++) {
                if (self.oBusData.TechGroup[i].TypeId === parseInt(dataId)) {
...
// chart 图表得编写
ES.MapView.AssetMonitor.include({
