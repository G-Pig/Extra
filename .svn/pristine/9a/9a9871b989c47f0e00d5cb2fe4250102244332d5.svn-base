...
ES.MapView.BaseTabPanel.FenceVehLst = ES.MapView.BaseTabPanel.BaseLst.extend({

    // 初始化界面
    initData: function (nPage, fnCall) {

        ES.Util.loadAn(this.$_oLstContainer);
        var oTemp = {vehNo: this.oSearchInput.val(),status: [1,2,3,4,5]};
        var _anDeptId = [];
        if (this.anDeptId && this.anDeptId.length > 0) {
            for(var i= 0;i<this.anDeptId.length;i++){
                if(this.anDeptId[i].indexOf("_") >= 0)_anDeptId.push(this.anDeptId[i].slice(1,this.anDeptId[i].length));
            }
            //this.anDeptId = _anDeptId;
            ES.extend(oTemp, {fenceIds: _anDeptId});
    ...
    initEvent:function () {
        ...
        setTimeout(function(){self.$_oOpenBtn.click();},500)
    },
...
ES.MapView.BaseTabPanel.FenceVehLst.include({
