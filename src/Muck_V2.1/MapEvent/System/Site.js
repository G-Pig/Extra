    ...
    drawZoomendSites:function(nKey) {
        ...
        if (nZoom <= 7) {
            ...
            for (var i = 0; i < aoSiteInfo.length; i++) {
                this._drawSiteMarker(aoSiteInfo[i], k, this.aoSiteInfoType);
            }
            for (var j = 0; j < aoStatusInfo.length; j++) {
                this._drawSiteMarker(aoStatusInfo[j], k, this.aoStatusInfoType);
            }
            for (var l = 0; l < aoSuspSiteInfo.length; l++) {
                this._drawSiteMarker(aoSuspSiteInfo[l], k, this.aoSuspSiteInfoType);
            }
            for (var b = 0; b < aoBayonetInfo.length; b++) {
                this._drawSiteMarker(aoBayonetInfo[b], k, this.aoBayonetInfoType);
            }
        }
        else {
            this._oSiteGroup.clearLayers();
            ...
            for (var i = 0; i < aoSiteInfo.length; i++) {
                this._drawPolygonSite(aoSiteInfo[i], k, this.aoSiteInfoType);
            }
            for (var j = 0; j < aoStatusInfo.length; j++) {
                this._drawPolygonSite(aoStatusInfo[j], k, this.aoStatusInfoType);
            }
            for (var b = 0; b < aoBayonetInfo.length; b++) {
                this._drawPolygonSite(aoBayonetInfo[b], k, this.aoBayonetInfoType);
            }
        }
    },
        ...
        switch(type){
            case this.oOption.site:
                ...
                oMarker.on('click', function () {
                    var oPop = new ES.Muck.PopSiteInfo(this, oPosInfo);
                    oPop.showModal();
                }, this);
    ...
    _drawPolygonSite: function (oPosInfo, nIndex,type) {
        ...
        switch(type){
            case this.oOption.site:
                ...
                oPolygon.bindTooltip(oInfo.Name);
                oPolygon.on('click', function () {
                    var oPop = new ES.Muck.PopSiteInfo(this, oInfo);
                    oPop.showModal();
                }, this);
