...
ES.SelectTruck.Dialog = ES.Evented.extend({
    ...
    initUI: function () {
        ...
        this.oGrid = new ES.SelectTruck.Grid(this, {
            ...
            multiselect:true,
            url:'/Vehicle/GetListByResourceTypeId',
            rowNum: 100000,
        });
    ...
    initEvent: function () {
        // if($('.ex-select-type-box>ul>li').find('a.all').hasClass('active')){
        //     $('.ui-dialog-button').find('button').eq(0).attr('disabled',false);
        // }
        if (!this.oDialog) {
            return;
        }
    ...
    initIsAllEvent:function(){
        ...
        $('.ex-select-type-box a.All').on('click',function(){
            ...
            self.oDialog.reset();
            //$('.ui-dialog-button').find('button').eq(0).attr('disabled',true);
        });
